...
class GradCAM:
    ...
    def _get_cam(self, non_image_data_single: torch.Tensor, image_data_single: torch.Tensor, target_class_idx: int = None) -> tuple:
        """
        ...
        if len(activations.shape) == 5:
            ...
            cam = self._compute_patch_based_cam(activations, gradients, image_data_single)
        elif len(activations.shape) == 2:
            logger.debug(f"Processing pooled activations: {activations.shape}")
            cam = self._compute_pooled_cam(activations, gradients, image_data_single)
        else:
    ...
    def _compute_patch_based_cam(self, activations, gradients, image_data_single):
        """
        Compute CAM from spatial convolutional activations and reconstruct to original image size.
        """      
        batch_patches, channels, act_h, act_w, act_d = activations.shape
        
        relu_grads = torch.relu(gradients)
        weights = torch.mean(relu_grads, dim=[2, 3, 4])
        
        for i in range(batch_patches):
            patch_weights = weights[i]
            weight_norm = torch.norm(patch_weights) + 1e-7
            weights[i] = patch_weights / weight_norm
        
        patch_cams = []
        for i in range(batch_patches):
            patch_cam = torch.zeros((act_h, act_w, act_d), device=activations.device)
            for c in range(channels):
                patch_cam += weights[i, c] * activations[i, c]
            patch_cam = torch.relu(patch_cam)
            if patch_cam.max() > 1e-6:
                patch_cam = patch_cam / patch_cam.max()
            patch_cams.append(patch_cam)
        
        reconstructed_cam = self._reconstruct_cam_from_patches(patch_cams, image_data_single)
        return reconstructed_cam.cpu().numpy()

    def _compute_pooled_cam(self, activations, gradients, image_data_single):
        """
        Compute CAM from pooled activations (less spatial information available).
        """
        patch_importances = torch.sum(torch.abs(gradients * activations), dim=1)
...
class GradCAMGenerator:
    ...
    def __init__(self, model: nn.Module, device: torch.device, target_branch_idx: int = 0, class_names: list = None):
        ...
        self.grad_cam_instance = GradCAM(self.model, self.target_branch_idx)
        
        self.hooks_available = len(self.grad_cam_instance.hook_handles) > 0
    ...
    def generate_heatmap_for_sample(self, non_image_data: torch.Tensor, image_data: torch.Tensor, target_class: int = None) -> tuple:
        ...
        heatmap_np, predicted_class_idx, confidence = self.grad_cam_instance._get_cam(
            non_image_data_single, image_data_single, target_class
        )
        return heatmap_np, predicted_class_idx, confidence

    def _plot_grad_cam_visualization(self, original_image_3d: np.ndarray, heatmap_3d_np: np.ndarray,
        ...
        original_image_3d: (C, H, W, D) numpy array
        heatmap_3d_np: (H, W, D) numpy array (CAM output, needs slicing)
        """
        ...
        if heatmap_3d_np is None or heatmap_3d_np.size == 0:
            logger.warning(f"Invalid or empty heatmap for sample {sample_idx}. Skipping plot.")
            return
        ...
        try:
            ...
            logger.error(f"Error resizing heatmap for sample {sample_idx}: {e}. Heatmap shape: {heatmap_3d_np.shape}, Target: {(img_h, img_w, img_d)}")
            return
        ...
        pred_class_name = self.class_names[pred_class_idx] if pred_class_idx < len(self.class_names) else f"Class {pred_class_idx}"
        true_class_name = self.class_names[true_label_idx] if true_label_idx < len(self.class_names) else f"Class {true_label_idx}"
        fig.suptitle(f"Grad-CAM Sample {sample_idx} - Pred: {pred_class_name} ({confidence:.2f}), True: {true_class_name}", fontsize=14)
        ...
        plot_path = os.path.join(output_dir, f"grad_cam_sample_{sample_idx}.png")
        try:
    ...
    def process_batch(self, non_image_data_batch: torch.Tensor, image_data_batch: torch.Tensor, labels_batch: torch.Tensor, 
        ...
        if not self.grad_cam_instance.hook_handles:
            logger.debug("Re-registering GradCAM hooks before processing batch.")
            self.grad_cam_instance._register_hooks()
            self.hooks_available = len(self.grad_cam_instance.hook_handles) > 0
            if not self.hooks_available:
            ...
            try:
                if current_image_sample is not None:
                    ...
                    if heatmap_np is not None:
                        ...
                        self._plot_grad_cam_visualization(original_image_for_plot, heatmap_np,
    ...
    def cleanup(self):
        """Removes hooks."""
        logger.debug("Cleaning up GradCAMGenerator: removing hooks.")
        self.grad_cam_instance.remove_hooks() 