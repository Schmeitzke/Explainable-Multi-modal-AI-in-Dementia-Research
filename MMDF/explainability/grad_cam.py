...
class GradCAM:
    ...
    def _get_cam(self, mri_data_single: torch.Tensor, clinical_data_single: torch.Tensor, target_class_idx: int = None) -> tuple[np.ndarray | None, int, float]:
        """
        ...
        pooled_gradients = torch.mean(self.activations.grad, dim=[0, 2, 3])
        feature_maps = self.activations.detach().squeeze(0)

        if pooled_gradients.shape[0] != feature_maps.shape[0]:
            logger.error(f"Mismatch in channels between pooled gradients ({pooled_gradients.shape[0]}) and feature maps ({feature_maps.shape[0]}).")
            confidence = torch.max(torch.softmax(target_logits_for_explanation, dim=1)).item()
            return None, target_class_idx, confidence
...
class GradCAMGenerator:
    ...
    def __init__(self, model: nn.Module, device: torch.device, target_layer_module: nn.Module, class_names: list[str] | None = None, model_modality_mode: str = "full"):
        ...
        self.grad_cam_instance = GradCAM(self.model, self.target_layer_module, self.model_modality_mode)
    ...
    def _plot_grad_cam_visualization(self, original_mri_slices: np.ndarray, heatmap_np: np.ndarray,
        ...
        original_mri_slices: (3, H, W) numpy array
        heatmap_np: (H', W') numpy array (CAM output, needs resizing)
        """
        ...
        for i in range(num_slices):
            ...
            if resized_heatmap.max() > 1e-6:
                resized_heatmap = resized_heatmap / resized_heatmap.max()
            else:
        ...
        for i in range(min(batch_size, samples_to_process_in_this_batch)):
            ...
            try:
                ...
                if heatmap_np is not None:
                    ...
                    self._plot_grad_cam_visualization(original_mri_for_plot, heatmap_np,
